    ...
    <div ref={wrapRef} style={{ minHeight: '100vh', background: bg, display: 'flex', flexDirection: 'column', transition: 'background 0.3s', fontFamily: 'system-ui,sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', position: 'relative', zIndex: 5 }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', maxWidth: 360 }}>
          ...
          <div style={{ width: 56, height: 56, borderRadius: 16, border: `2px solid rgba(251,191,36,0.3)`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18, background: 'rgba(251,191,36,0.08)' }}>
            <i className="ti ti-alert-triangle" style={{ fontSize: 26, color: '#fbbf24' }} />
          </div>

          <h1 style={{ fontSize: 18, fontWeight: 900, color: textPrimary, margin: '0 0 8px', letterSpacing: '-0.2px' }}>
            Terjadi kesalahan
          </h1>
          <p style={{ fontSize: 13, color: textSecondary, lineHeight: 1.6, margin: '0 0 24px' }}>
            Ada yang tidak beres. Coba muat ulang halaman atau kembali ke dashboard.
          </p>

          {error?.message && (
            <div style={{ width: '100%', border: `0.5px solid ${cardBorder}`, borderRadius: 10, padding: '10px 14px', background: cardBg, marginBottom: 24, textAlign: 'left' }}>
              <div style={{ fontSize: 10, color: textTertiary, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Detail error</div>
              <div style={{ fontSize: 11, color: textSecondary, fontFamily: 'monospace', wordBreak: 'break-all' }}>{error.message}</div>
            </div>
          )}

          <div ref={btnGroupRef} style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center' }}>
            <button onClick={reset} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 22px', borderRadius: 22, border: 'none', background: 'linear-gradient(90deg,#FFD600,#FF7A00,#FF0069,#D300C5,#7638FA)', backgroundSize: '200% 100%', animation: 'igShift 4s ease infinite', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          ...
          <div style={{ marginTop: 32, fontSize: 11, color: textTertiary }}>
            Kalau ini terus terjadi, hubungi kami ya.
          </div>
        </div>
      </div>
    </div>
