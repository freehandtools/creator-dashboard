...
export default function NotFound() {
  ...
  return (
    <>
    <title>Halaman Tidak Ditemukan — Creator Performance Intelligence Dashboard</title>
    <div ref={wrapRef} style={{ minHeight: '100vh', background: bg, display: 'flex', flexDirection: 'column', transition: 'background 0.3s', fontFamily: 'system-ui,sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 1, mixBlendMode: nebulaMix as React.CSSProperties['mixBlendMode'] }}>
        <div ref={nebula1Ref} style={{
          position: 'absolute', bottom: '-15%', left: '50%',
          transform: 'translateX(-50%)', width: '140%', height: '50%',
          filter: 'blur(65px)', opacity: 0, transition: 'opacity 0.25s ease, transform 0.4s ease',
          background: 'radial-gradient(ellipse 50% 100% at 50% 100%, #FFD600 0%, #FF7A00 22%, #FF0069 45%, #D300C5 68%, #7638FA 90%, transparent 100%)',
        }} />
        <div ref={nebula2Ref} style={{
          position: 'absolute', bottom: '-15%', left: '50%',
          transform: 'translateX(-50%)', width: '140%', height: '42%',
          filter: 'blur(65px)', opacity: 0, transition: 'opacity 0.25s ease, transform 0.4s ease',
          background: 'radial-gradient(ellipse 40% 100% at 50% 100%, #7638FA 0%, #D300C5 35%, #FF0069 65%, transparent 90%)',
        }} />
      </div>
          ...
          {Array.from({ length: 100 }, (_, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                boxShadow: size > 2 ? `0 0 4px ${starShadow}` : 'none'
              }} />
            )
          })}
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', position: 'relative', zIndex: 5 }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', maxWidth: 360 }}>
          ...
          <div style={{ fontSize: 96, fontWeight: 900, lineHeight: 1, marginBottom: 8, animation: 'float 4s ease-in-out infinite' }}>
            <span style={{ background: 'linear-gradient(90deg,#FFD600,#FF7A00,#FF0069,#D300C5,#7638FA)', backgroundSize: '300% 100%', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', animation: 'igShift 4s ease infinite' }}>
              404
            </span>
          </div>

          <div style={{ fontSize: 18, fontWeight: 700, color: textPrimary, marginBottom: 10 }}>
            Halaman tidak ditemukan
          </div>
          <div style={{ fontSize: 13, color: textSecondary, lineHeight: 1.6, marginBottom: 32 }}>
            URL yang kamu akses tidak ada atau sudah dipindahkan.
          </div>

          {/* Tombol — ini yang jadi acuan jarak cursor */}
          <div ref={btnGroupRef} style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center' }}>
            <Link href="/dashboard" style={{ textDecoration: 'none' }}>
              <button style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 22px', borderRadius: 22, border: 'none', background: 'linear-gradient(90deg,#FFD600,#FF7A00,#FF0069,#D300C5,#7638FA)', backgroundSize: '200% 100%', animation: 'igShift 4s ease infinite', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                <i className="ti ti-layout-dashboard" style={{ fontSize: 15 }} />
                Ke Dashboard
              </button>
            </Link>
            <Link href="/" style={{ textDecoration: 'none' }}>
              <button style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 22px', borderRadius: 22, border: `0.5px solid ${navBorder}`, background: cardBg, color: textPrimary, fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
          ...
          <div style={{ marginTop: 40, fontSize: 11, color: textTertiary }}>
            Kalau ini bug, hubungi kami ya.
          </div>
        </div>
      </div>
    </div>
